...
get_stock_data <- function(stock_symbol, 
  ...
  stock_symbol %>% quantmod::getSymbols( 
    ...
    dplyr::mutate(mavg_short = zoo::rollmean(adjusted, mavg_short, fill = NA, align = "right")) %>% 
    dplyr::mutate(mavg_long  = zoo::rollmean(adjusted, mavg_long,  fill = NA, align = "right")) %>% 
